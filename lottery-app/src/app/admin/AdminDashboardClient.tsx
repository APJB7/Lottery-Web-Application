        ...
        <div className="overflow-hidden rounded-[32px] bg-gradient-to-r from-emerald-700 to-cyan-700 px-8 py-8 text-white shadow-2xl">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-4xl font-semibold tracking-tight">Admin Review Dashboard</h1>
              <p className="mt-2 text-white/85">Review receipts, verify details, and approve or reject entries.</p>
            </div>
            <button onClick={logout} className="rounded-full bg-white px-5 py-2 text-sm font-medium text-slate-900 hover:bg-slate-100">
              Logout
            </button>
          </div>

          <div className="mt-6 flex flex-wrap gap-2">
            {["ALL", "PENDING_PAYMENT", "PENDING_REVIEW", "AUTO_VERIFIED", "APPROVED", "REJECTED"].map((value) => (
              <button
                key={value}
                onClick={() => setFilter(value)}
                className={`rounded-full px-4 py-2 text-sm font-medium ${filter === value ? "bg-white text-slate-900" : "bg-white/15 text-white hover:bg-white/25"}`}
              >
                {value}
              </button>
            ))}
                ...
                <div className="grid gap-6 xl:grid-cols-[1fr,280px]">
                  <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    <div className="space-y-2 rounded-3xl bg-slate-50 p-5">
                      ...
                      <p><span className="font-medium text-slate-900">Status:</span> <span className="rounded-full bg-gradient-to-r from-emerald-600 to-cyan-600 px-3 py-1 text-sm text-white">{entry.status}</span></p>
                    </div>
